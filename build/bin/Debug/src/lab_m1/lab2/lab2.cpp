...
void Lab2::Init()
{
    ...
    // Create a mesh box using custom data
    {
        vector<VertexFormat> vertices
        {
            ...
            */
            ...
            // argumente: VertexFormat(pozitie, normale, culoare)
        ...
        vector<unsigned int> indices =
        {
            ...
            // Un cub are 6 fete (patrate)
            // Un patrate se genereaza din 2 triunghiuri
            // ==> Se vor genera 6*2 = 12 triunghiuri
            ...
            */
            ...
            // Patratul pt fata din fata (dinpre noi) a cubului
    ...
    // by definition!
    {
        ...
        {
            VertexFormat(glm::vec3(0, 1, 0), glm::vec3(1, 0, 0)), // Top vertex
            VertexFormat(glm::vec3(-1, -1, -1), glm::vec3(0, 1, 0)), // Bottom Left vertex
            VertexFormat(glm::vec3(1, -1, -1), glm::vec3(0, 0, 1)), // Bottom Right vertex
            VertexFormat(glm::vec3(0, -1, 1), glm::vec3(1, 1, 0)) // Bottom Front vertex
        };
    ...
    {
        vector<VertexFormat> triangle1Vertices
        {
            VertexFormat(glm::vec3(-1, -1, 0), glm::vec3(1, 0, 0)), // Bottom Left
            VertexFormat(glm::vec3(1, -1, 0), glm::vec3(0, 1, 0)),  // Bottom Right
            VertexFormat(glm::vec3(1, 1, 0), glm::vec3(0, 0, 1)),   // Top Right
        };

        vector<VertexFormat> triangle2Vertices
        {
            VertexFormat(glm::vec3(-1, -1, 0), glm::vec3(1, 0, 0)), // Bottom Left
            VertexFormat(glm::vec3(1, 1, 0), glm::vec3(0, 1, 0)),   // Top Right
            VertexFormat(glm::vec3(-1, 1, 0), glm::vec3(0, 0, 1)),  // Top Left
        };

        vector<unsigned int> triangle1Indices = { 0, 1, 2 }; // First triangle (front-facing)
        vector<unsigned int> triangle2Indices = { 0, 2, 1 }; // Second triangle (back-facing)

        // Create two meshes, one for each triangle
        meshes["triangle1"] = new Mesh("triangle1");
        meshes["triangle1"]->InitFromData(triangle1Vertices, triangle1Indices);

        meshes["triangle2"] = new Mesh("triangle2");
        meshes["triangle2"]->InitFromData(triangle2Vertices, triangle2Indices);
    }
...
void Lab2::Update(float deltaTimeSeconds)
{
    ...
    // Render the new object (cube_B) at a different position to avoid overlap
    RenderMesh(meshes["cube_B"], shaders["VertexColor"], glm::vec3(1.5f, 0.5f, 0), glm::vec3(0.25f)); // New cube_B
    ...
    // First triangle
    RenderMesh(meshes["triangle1"], shaders["VertexColor"], glm::vec3(0.f, 3.f, 0.f));

    // Second triangle
    RenderMesh(meshes["triangle2"], shaders["VertexColor"], glm::vec3(0.f, 3.f, 0.f));
...
void Lab2::OnKeyPress(int key, int mods)
{
    ...
    if (key == GLFW_KEY_F2)
    {
        ...
        // mai destept ca mai sus: cullFace = cullFace ^ GL_BACK ^ GL_FRONT;
    }
