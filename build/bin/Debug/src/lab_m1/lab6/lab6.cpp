...
Mesh* Lab6::CreateMesh(const char* name, const std::vector<VertexFormat>& vertices, const std::vector<unsigned int>& indices)
{
    ...
    /*
    Observatia mea: dupa schimbarea color pipe-urilor, culorile par alterate

    Daca dau switch la culoare in shader, dar nu in functie:
    cubul din stanga se vede cu negru,
    iar cel din dreapta are culorile alterate (un pic de albastru-turcoaz in capatul din spate-jos)

    Sunt mai multe combinatii posibile (cred ca 8 = 2x2x2; 2 din lab6.cpp, 2 din FragmentShaer.glsl, 2 din VertexShader.glsl)
    Try and error
    ...
    */
    ...
    // Pipe 1
    glEnableVertexAttribArray(1);   // MyTODO: initial este 1, schimb cu 3
    ...
    // Pipe 3
    glEnableVertexAttribArray(3);   // MyTODO: initial este 3, schimb cu 1
...
void Lab6::RenderSimpleMesh(Mesh* mesh, Shader* shader, const glm::mat4& modelMatrix)
{
    ...
    // myTODO: se trimite timpul aplicatiei
   // GLint appTime = glGetUniformLocation(shader->GetProgramID(), "Time");
   // glUniform1f(appTime, (GLfloat)Engine::GetElapsedTime());
