...
Texture2D* Lab9::CreateRandomTexture(unsigned int width, unsigned int height)
{
    ...
    /*
    //  Un Random cu gradient
    for (unsigned int y = 0; y < height; ++y) {
        for (unsigned int x = 0; x < width; ++x) {
            unsigned int index = (y * width + x) * channels;
            float t = static_cast<float>(x) / (width - 1); // Horizontal gradient factor

            data[index] = static_cast<unsigned char>((1 - t) * 255); 
            data[index + 1] = static_cast<unsigned char>(t * 255);  
            data[index + 2] = static_cast<unsigned char>((1 - t) * 128 + t * 255);
        }
    }
    */
